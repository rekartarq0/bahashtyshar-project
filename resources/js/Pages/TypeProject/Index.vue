...
import CategoryTable from "./TypeProjectTable.vue";
...
function submit(event) {
    ...
    const url = form.id ? `/typemulk/${form.id}` : '/typemulk'; // Determine URL
...
const handleSearch = debounce(() => {
    router.get('/typemulk', { q: searchQuery.value }, {
...
<template>
    <AppLayout title="typemulk Index">
        <template #header>
    ...
    <el-dialog class="font-droidkufi" dir="rtl" v-model="dialogFormVisible" title="فۆڕمی زیاکردنی جۆری موڵک" width="700"
        align-center="true" :close-on-click-modal="false">
        <el-form :model="form" @keydown.enter="submit">
            <el-form-item :error="ErrorForms.name" label-position="left" label="ناوی پۆلێن"
