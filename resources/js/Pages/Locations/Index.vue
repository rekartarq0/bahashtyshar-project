...
function submit(event) {
    ...
    const url = form.id ? `/locations/${form.id}` : '/locations'; // Determine URL
...
const handleSearch = debounce(() => {
    router.get('/locations', { q: searchQuery.value }, {
...
<template>
    <AppLayout title="locations Index">
        <template #header>
        ...
        <LocationsTable :data="data" :destroy="destroy" :edit="edit" />

    </AppLayout>

    <el-dialog class="font-droidkufi" dir="rtl" v-model="dialogFormVisible" title="فۆڕمی زیاکردنی ناونیشان" width="700"
